...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::HTK)
    {
    }
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeHTKAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeHTKAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeHTKAmount = strAnonymizeHTKAmount.remove(strAnonymizeHTKAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHTKAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeHTKAmount * COIN) nMaxToAnonymize = nAnonymizeHTKAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeHTKAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeHTKAmount));
        strAnonymizeHTKAmount = strAnonymizeHTKAmount.remove(strAnonymizeHTKAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHTKAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
