...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 59200, 55600));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, htk, (Darksend, Instantx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all HTK specific functionality (Masternodes, Darksend, InstantX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:59200"));
    ...
    strUsage += HelpMessageOpt("-anonymizehtkamount=<n>", strprintf(_("Keep N HTK anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u)"), 59100));
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizeHTKAmount = GetArg("-anonymizehtkamount", 0);
    if (nAnonymizeHTKAmount > 999999) nAnonymizeHTKAmount = 999999;
    if (nAnonymizeHTKAmount < 2) nAnonymizeHTKAmount = 2;
    ...
    LogPrintf("Anonymize HTK Amount %d\n", nAnonymizeHTKAmount);
    ...
    /* Denominations
       ...
       For example:
       1HTK+1000 == (.1HTK+100)*10
       10HTK+10000 == (1HTK+1000)*10
    */
